...
fn wake_during_blocked_poll() {
    check_dfs(
        || {
            ...
            // If this `wake()` invocation happens while thread 1 holds the `counter` lock and the
            // `block_on` task is blocked waiting to acquire that lock, it must not unblock the
            // `block_on` task. That is, `wake` should prevent the task from being blocked *the next
            // time it returns Pending*, not just any time it is blocked.
            thread::spawn(move || {
                if let Some(waker) = waker_clone.lock().unwrap().take() {
                    waker.wake();
                }
